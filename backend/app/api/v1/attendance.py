...
def take_attendance(
    ...
    existing = db.query(AttendanceRecord).filter(
        AttendanceRecord.class_arm_id == payload.class_arm_id, AttendanceRecord.date == payload.date
    ).first()
    if existing:
    ...
    for entry in payload.entries:
        ...
        db.add(AttendanceRecord(
            ...
            academic_term_id=payload.academic_term_id, class_arm_id=payload.class_arm_id,
...
def attendance_history(
    ...
    if class_arm_id:
        q = q.filter(AttendanceRecord.class_arm_id == class_arm_id)
    if student_id:
...
@router.patch("/class/{class_arm_id}/lock", response_model=ApiResponse)
def lock_attendance(
    class_arm_id: UUID,
    date: date_type,
    db: Session = Depends(get_db),
    ...
    db.query(AttendanceRecord).filter(
        AttendanceRecord.class_arm_id == class_arm_id, AttendanceRecord.date == date
    ).update({AttendanceRecord.is_locked: True})
    db.commit()
    return ApiResponse(success=True, message="Attendance window locked.")


@router.patch("/class/{class_arm_id}/reopen", response_model=ApiResponse)
