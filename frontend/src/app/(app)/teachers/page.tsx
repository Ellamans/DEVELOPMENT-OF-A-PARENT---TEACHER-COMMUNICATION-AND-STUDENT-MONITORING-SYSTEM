...
function AssignClassModal({ teacher, onClose }: { teacher: Teacher; onClose: () => void }) {
  ...
  const willReplace = !!selectedArm?.class_teacher_id && selectedArm.class_teacher_id !== teacher.user_id;

  async function handleAssign() {
    if (!armId) {
      toast.error("Pick a class and arm first.");
      return;
    }
    setIsSubmitting(true);
    try {
      await apiClient.patch(`/school-setup/class-arms/${armId}`, { class_teacher_id: teacher.user_id });
      toast.success(`${teacher.full_name} is now the class teacher for ${selectedArm?.full_name}.`);
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md bg-card border border-border rounded-lg p-6">
        ...
        <p className="text-xs text-text/50 mb-4">
          This makes {teacher.full_name} the class teacher for the selected class arm — e.g. "Teacher 1 → JSS 1A".
        ...
        <div className="space-y-3">
          ...
          {willReplace && (
            <p className="text-xs text-orange-600 bg-orange-500/10 rounded p-2">
              {selectedArm?.class_teacher_name} is currently the class teacher for {selectedArm?.full_name}.
...
export default function TeachersPage() {
  ...
  function classArmsFor(userId: string) {
    return (armsQuery.data ?? []).filter((a) => a.class_teacher_id === userId);
  }
  ...
  return (
    <div>
      ...
      <p className="text-sm text-text/60 mb-4">
        ...
        make a teacher the class teacher for a class arm (e.g. Teacher 1 → JSS 1) — once assigned, parents and
              ...
              {data.data.map((t) => (
                <tr key={t.id} className="border-t border-border hover:bg-border/10">
                  ...
                  <td className="px-4 py-3">
                    {classArmsFor(t.user_id).length ? (
                      classArmsFor(t.user_id).map((a) => a.full_name).join(", ")
                    ) : (
