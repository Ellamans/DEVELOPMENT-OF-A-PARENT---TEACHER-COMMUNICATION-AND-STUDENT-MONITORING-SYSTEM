...
export default function ReportCardsPage() {
  ...
  const cardsQuery = useQuery({
    ...
    queryFn: async () => (await apiClient.get("/report-cards", { params: { class_id: classId, academic_term_id: termId } })).data.data as ReportCardRow[],
  });
  ...
  async function computeResults() {
    ...
    try {
      const { data } = await apiClient.post("/report-cards/compute-class", {
        class_id: classId, academic_session_id: sessionId, academic_term_id: termId,
      });
  ...
  return (
    <div>
      ...
      <button
        ...
        disabled={isComputing || !classId || !sessionId || !termId}
      ...
      {!classId || !termId ? (
        <div className="bg-card border border-border rounded-lg py-16 text-center text-text/50 text-sm">Select a class and term to view report cards.</div>
