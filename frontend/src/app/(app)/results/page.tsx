...
function CATab() {
  ...
  const studentsQuery = useQuery({
    queryKey: ["students-for-ca", classId],
    enabled: !!classId,
    queryFn: async () => (await apiClient.get("/students", { params: { class_id: classId, page_size: 100 } })).data.data as Student[],
  });
  const existingQuery = useQuery({
    queryKey: ["ca-existing", classId, subjectId, termId],
    enabled: !!classId && !!subjectId && !!termId,
    queryFn: async () => (await apiClient.get("/continuous-assessments", { params: { class_id: classId, subject_id: subjectId, academic_term_id: termId } })).data.data as CAEntry[],
  });
  ...
  async function saveScore(studentId: string) {
    ...
    if (!classId || !subjectId || !sessionId || !termId || !componentId) {
      toast.error("Select a class, subject, session, term, and component first.");
      return;
    }
  ...
  return (
    <div>
      <div className="bg-card border border-border rounded-lg p-4 mb-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        <select value={classId} onChange={(e) => setClassId(e.target.value)} className="rounded border border-border bg-background px-3 py-2 text-sm text-text">
      ...
      {!classId || !subjectId || !termId || !componentId ? (
        ...
        <div className="bg-card border border-border rounded-lg py-16 text-center text-text/50 text-sm">No students in this class.</div>
      ) : (
...
function ExamTab() {
  ...
  const studentsQuery = useQuery({
    queryKey: ["students-for-exam", classId],
    enabled: !!classId,
    queryFn: async () => (await apiClient.get("/students", { params: { class_id: classId, page_size: 100 } })).data.data as Student[],
  });
  const examsQuery = useQuery({
    queryKey: ["exam-results", classId, subjectId, termId],
    enabled: !!classId && !!subjectId && !!termId,
    queryFn: async () => (await apiClient.get("/exam-results", { params: { class_id: classId, subject_id: subjectId, academic_term_id: termId } })).data.data as ExamEntry[],
  });
  ...
  async function saveScore(studentId: string) {
    ...
    if (!classId || !subjectId || !sessionId || !termId) {
      toast.error("Select a class, subject, session, and term first.");
      return;
    }
    setSavingId(studentId);
    try {
