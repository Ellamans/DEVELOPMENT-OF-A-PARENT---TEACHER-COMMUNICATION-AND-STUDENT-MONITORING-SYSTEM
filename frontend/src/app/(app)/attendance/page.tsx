...
export default function AttendancePage() {
  ...
  const studentsQuery = useQuery({
    queryKey: ["students-for-attendance", classId],
    enabled: !!classId,
    queryFn: async () => {
      const { data } = await apiClient.get("/students", { params: { class_id: classId, page_size: 100 } });
  ...
  const historyQuery = useQuery({
    ...
    enabled: mode === "history" && !!classId,
    queryFn: async () => {
      const { data } = await apiClient.get("/attendance", { params: { class_id: classId, date_from: date, date_to: date } });
  ...
  async function submitAttendance() {
    if (!classId || !sessionId || !termId) {
      toast.error("Select a class, session, and term first.");
      return;
    }
  ...
  async function toggleLock(lock: boolean) {
    if (!classId) return;
    try {
      await apiClient.patch(`/attendance/class/${classId}/${lock ? "lock" : "reopen"}`, null, { params: { date } });
    ...
    <div>
      ...
      {!classId ? (
        <div className="bg-card border border-border rounded-lg py-16 text-center text-text/50 text-sm">
          Select a class to {mode === "take" ? "take attendance" : "view history"}.
        </div>
      ) : mode === "take" ? (
        studentsQuery.isLoading ? (
          <div className="flex justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
        ) : !studentsQuery.data?.length ? (
          <div className="bg-card border border-border rounded-lg py-16 text-center text-text/50 text-sm">No students in this class yet.</div>
        ) : (
          <div>
