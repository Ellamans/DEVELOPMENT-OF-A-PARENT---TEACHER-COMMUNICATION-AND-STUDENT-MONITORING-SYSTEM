...
function CreateAssignmentModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    title: "", instructions: "", subject_id: "", class_arm_id: "", teacher_id: "",
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.title.trim() || !form.subject_id || !form.class_arm_id || !form.teacher_id || !form.due_date) {
      toast.error("Title, subject, class arm, teacher, and due date are required.");
      return;
    }
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-lg p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <select value={classId} onChange={(e) => { setClassId(e.target.value); setForm({ ...form, class_arm_id: "" }); }} className="w-full rounded border border-border bg-background px-3 py-2 text-text">
          ...
          <div>
            ...
            <select value={form.class_arm_id} onChange={(e) => setForm({ ...form, class_arm_id: e.target.value })} className="w-full rounded border border-border bg-background px-3 py-2 text-text">
